...
present_date = str(datetime.utcnow() + timedelta(hours = 9))[:10] #UTC + 9시간
...
# 데이터 전처리
df = pd.DataFrame(book_list, columns = ["aladin_순위", "제목", "aladin_저자", "aladin_출판사", "aladin_출간일", "aladin_가격" "aladin_평점", "aladin_장르", "상명대학교_학술정보관_장르", "present_date"])
...
for k in range(len(df2)) :
    es.index(
        ...
        "aladin_순위" : str(df2.loc[k].aladin_순위),
         ...
         "present_date" : str(df2.loc[k].present_date)
         })

df = pd.DataFrame(book_list, columns = ["aladin_순위", "제목", "aladin_저자", "aladin_출판사", "aladin_출간일", "aladin_가격", "aladin_평점", "aladin_장르", "상명대학교_학술정보관_장르", "present_date"])
# df.to_csv("알라딘_장르top100_11개_연습용 0223" + '.csv', index = False, encoding = 'utf-8-sig')
df.to_csv("알라딘_장르top100_11개_연습용 0223_datetime" + '.csv', index = False, encoding = 'utf-8-sig')
print(df)